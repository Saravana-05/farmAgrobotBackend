    ...
    try:
        ...
        def calculate_metrics(sales_queryset):
            metrics = sales_queryset.aggregate(
                ...
                average_paid_amount=Avg('paid_amount'),
                total_commission=Sum('commission'),
                total_deductions=Sum('total_deductions')
            )
            ...
            if metrics['total_sales_amount'] > 0:
                metrics['collection_rate'] = round(
                    (metrics['total_paid'] / metrics['total_sales_amount']) * 100, 2
                )
            else:
                metrics['collection_rate'] = 0
            
            return {
                'total_revenue': float(metrics['total_revenue']),  # Paid amount
                'total_sales_amount': float(metrics['total_sales_amount']),  # Final amount
                ...
                'average_paid_amount': round(float(metrics['average_paid_amount']), 2),
        ...
        time_periods['last_7_days'] = {
            **calculate_metrics(last_7_days_sales),
            ...
            'start_date': last_7_days_start.isoformat(),
            'end_date': last_7_days_end.isoformat()
        }
        ...
        time_periods['last_30_days'] = {
            **calculate_metrics(last_30_days_sales),
            ...
            'start_date': last_30_days_start.isoformat(),
            'end_date': last_30_days_end.isoformat()
        }
        ...
        response_data = {
            ...
            'note': 'Revenue represents actual collected payments (paid_amount)'
        }
