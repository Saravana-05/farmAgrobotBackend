    ...
    try:
        ...
        def get_crop_data_for_period(yields_queryset, period_name):
            ...
            for crop_data in crops_summary.values():
                ...
                # Calculate total quantity per unit AND overall total
                unit_totals = {}
                total_quantity_all_units = 0.0
                ...
                for variant in crop_data['variants']:
                    ...
                    if unit not in unit_totals:
                        unit_totals[unit] = 0.0
                    unit_totals[unit] += quantity
                    total_quantity_all_units += quantity
                ...
                # ✅ FIX: Add total_quantity at crop level
                crop_data['total_quantity'] = total_quantity_all_units
                
                # Add variant count
                crop_data['variant_count'] = len(crop_data['variants'])
                
                # Add units dictionary for easy access
                crop_data['units'] = unit_totals
        ...
        # ✅ FIX: Get crop metadata with images
        crop_metadata = []
        if crop_id:
            # Single crop filter
            try:
                ...
                image_url = None
                if crop.crop_image:
                    image_url = request.build_absolute_uri(crop.crop_image.url)
            ...
            # Get all crops that have yields in the filtered data
            crop_ids_with_yields = base_yields.values_list('crop_id', flat=True).distinct()
            crops = Crop.objects.filter(id__in=crop_ids_with_yields)
        ...
        return Response({
            ...
            'data': {
                ...
                'crop_metadata': crop_metadata  # ✅ FIX: Add crop metadata
            }
        }, status=status.HTTP_200_OK)
